...
export default function App() {
    ...
    if (data && data.length > 0) {
      ...
      const friendsWithData = await Promise.all(
        data.map(async (friend) => {
          ...
          const friendName = bookshelfData?.books?.[0]?.author || '好友';
          const bookCount = bookshelfData?.books?.length || 0;
  ...
  const viewFriendBookshelf = async (friend) => {
    ...
    if (userData?.data) {
      setFriendBookshelf(userData.data);
  ...
  return (<div className="app bookshelf-view"><div className="shelf-globe-bg" style={{ transform: `translateX(-50%) translateY(${-shelfOverscroll}px)`, transition: shelfOverscroll === 0 ? 'transform 0.4s cubic-bezier(0.25, 0.46, 0.45, 0.94)' : 'none' }} onClick={() => setShowProfile(true)} /><header className="bookshelf-header">{addedFriends.length > 0 && <button className="friends-entry-btn" onClick={() => setShowFriendsList(true)}>🪐</button>}<h1>一页穹顶</h1><p className="subtitle">拾起每一颗星星</p><p className="subtitle">便能拥有属于你的宇宙</p><button className="search-star" onClick={() => setShowSearch(true)}>⭐</button></header><div className="bookshelf-carousel" ref={shelfRef} onScroll={(e) => {
    const el = e.target;
